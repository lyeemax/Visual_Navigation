                                 ...
                                 uchar* desc)
{
    ...
    #define GET_VALUE(idx) \
        center[cvRound(pattern[idx].x*b + pattern[idx].y*a)*step + cvRound(pattern[idx].x*a - pattern[idx].y*b)]                // x'
...
//下面就是预先定义好的随机点集，256是指可以提取出256bit的描述子信息，每个bit由一对点比较得来；4=2*2，
// 前面的2是需要两个点（一对点）进行比较，后面的2是一个点有两个坐标
    ...
    iniThFAST(_iniThFAST), minThFAST(_minThFAST)//设置这些参数
{
    ...
    //s=1/1.2
    //是这样推的：设第一层要提取t个点，第二层st个点，第三次s^2×t个点，最后第n层s^(n-1)*t 那么(t+st+s^2*t+......+s^(n-1)*t)=At=N
    //N是总共要提取的点，t就是N/A也就是nDesiredFeaturesPerScale了，等比数列求和

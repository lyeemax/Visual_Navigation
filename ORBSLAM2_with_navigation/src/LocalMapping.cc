...
void LocalMapping::InsertPointCloud(KeyFrame *KF,cv::Mat depthImg,cv::Mat rgbImg){
    unique_lock<mutex> lock(mMutexCloud);
    PointTCloudPtr cloud=Converter::rangeToPointCloud(depthImg,rgbImg,KF->mK);
    mlCloudPool.insert(make_pair(KF,cloud));

    unique_lock<mutex> lock1(mpMap->mMutexPcMap);
    auto out = boost::shared_ptr<PointTCloud>(new PointTCloud);
    auto trans = Converter::toIsometry(KF->GetPoseInverse());
    pcl::transformPointCloud(*cloud, *out, trans.matrix());
#pragma omp parallel for
    for (size_t j = 0; j < out->size(); ++j) {
        mpMap->mPointCloudMap->push_back(out->points[j]);
    }
    //cout<<"pointcloud size :"<<mlCloudPool.size()<<endl;
}

void  LocalMapping::GenerateFullPointCloudMap() {
    {
        {
            unique_lock<mutex> lock(mpMap->mMutexPcMap);
            mpMap->mPointCloudMap->clear();
            cout << "refresh cloud map" << endl;
//#pragma omp parallel for
            for (auto kfmap:mlCloudPool) {
                auto KF = kfmap.first;
                PointTCloudPtr cloud = kfmap.second;
                auto out = boost::shared_ptr<PointTCloud>(new PointTCloud);
                auto trans = Converter::toSE3Quat(KF->GetPoseInverse());
                pcl::transformPointCloud(*cloud, *out, trans.to_homogeneous_matrix());
                for (size_t j = 0; j < out->size(); ++j) {
                    mpMap->mPointCloudMap->push_back(out->points[j]);
                }
            }
        ...
        for (size_t i = 0; i < vpNeighKFs.size (); i++) {
            ...
            for (int ikp = 0; ikp < nmatches; ikp++) {
                ...
                float cosParallaxStereo =
                        cosParallaxRays + 1;  //一般相邻的关键帧应该不会有这么大的视角吧,90°多,不过感觉这个也不好说啊;不过下面的程序中认为如果视差角的余弦值大于1是不正常的
...
void  LocalMapping::PointCloudCulling(){
...
void LocalMapping::KeyFrameCulling()
{
    ...
    for(vector<KeyFrame*>::iterator vit=vpLocalKeyFrames.begin(), vend=vpLocalKeyFrames.end(); vit!=vend; vit++)
    {
        ...
        if(nRedundantObservations>0.5*nMPs){
            if(mlCloudPool.count(pKF) && pKF->isBad()){
                unique_lock<std::mutex> lock(mMutexCloud);
                mlCloudPool[pKF]->clear();
                mlCloudPool.erase(pKF);
            }
        }
